...
- (void)createEditButton
{
    ...
    self.editButton.center = CGPointMake(self.view.frame.size.width-DeltaRight, self.view.window.bounds.size.height-DeltaBottom);
...
{
     if (self.feedStyleList.count > 0)//有数据
     {
         self.noDataTipLabel.hidden = YES;
         if (self.haveNextPage)//有下一页
         {
...
- (void)customObj:(id)obj clickOnUser:(UMComUser *)user
{
    ...
    if ([self.fetchRequest isKindOfClass:[UMComUserFeedsRequest class]]) {
        ...
        if ([user.uid isEqualToString:userFeedRequest.fuid]) {
            return;
        }
    }
    ...
    for (int i = 0; i < sourceData.count; i++) {
        ...
        if (tempSourceFeedStyle && [tempSourceFeedStyle isKindOfClass:[UMComFeedStyle class]]) {
            ...
            if (sourceFeedStyle) {
                ...
                if (sourceFeedStyle.feed.is_topType.integerValue == EUMTopFeedType_None) {
...
- (void)handleCoreDataDataWithData:(NSArray *)data error:(NSError *)error dataHandleFinish:(DataHandleFinish)finishHandler
{
    if (!error && [data isKindOfClass:[NSArray class]]) {
        ...
        NSArray* sourceArray = [self transFormToFeedStylesWithFeedDatas:self.dataArray];
