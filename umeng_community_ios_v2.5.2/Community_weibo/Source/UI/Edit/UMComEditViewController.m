...
- (void)viewDidLoad
{
    ...
    [self setTitleViewWithTitle:UMComLocalizedString(@"um_com_micro_editTitle", @"新鲜事")];
...
- (void)createTextView
{
    ...
    self.realTextView.placeholderLabel.text = UMComLocalizedString(@"um_com_micro_edit_realTextViewPlaceholder", @" 写点什么吧");
    ...
    {
        self.realTextView.layoutManager.allowsNonContiguousLayout = NO;
    }
}
...
//        UILabel *label = [[UILabel alloc]initWithFrame:CGRectMake(3, 0, self.topicNoticeBgView.frame.size.width, 25)];
    ...
    if ([[[[UMComSession sharedInstance] loginUser] gender] integerValue] == 1) {
        label.text = UMComLocalizedString(@"um_com_micro_edit_addTopicTipForMale", @"大哥啊,添加个话题吧!");
    }else{
        label.text = UMComLocalizedString(@"um_com_micro_edit_addTopicTipForMale", @"大妹砸,添加个话题吧!");
    }
...
- (void)showWhenForwordOldFeed
{
    ...
    self.realTextView.placeholderLabel.text = UMComLocalizedString(@"um_com_editView_placeholder", @" 说说你的观点...");;
    ...
    if (feedString && [feedString isEqualToString:@""] && originFeed.image_urls.count > 0) {
        feedString = UMComLocalizedString(@"um_com_shareImage", @"分享图片");
    }
...
//        UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:UMComLocalizedString(@"um_com_sorry", @"抱歉")  message:UMComLocalizedString(@"Empty_Text",@"文字内容不能为空") delegate:nil cancelButtonTitle:UMComLocalizedString(@"um_com_ok",@"好") otherButtonTitles:nil];
//        [alertView show];
//        [self.realTextView becomeFirstResponder];
//        return;
//    }
    ...
    if (!self.forwardFeed && ((self.realTextView.text.length == 0) && (self.originImages.count == 0))) {
        ...
        UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:UMComLocalizedString(@"um_com_sorry", @"抱歉")  message:UMComLocalizedString(@"The content or img",tooShortNotice) delegate:nil cancelButtonTitle:UMComLocalizedString(@"um_com_ok",@"好") otherButtonTitles:nil];
        [alertView show];
        [self.realTextView becomeFirstResponder];
        return;
    }
    ...
    if (!self.forwardFeedBackground) {
        if (self.topic) {
            ...
            //此处不管设置，都需要加入当前传入编辑框的话题 version 2.5
            [self topicsAddOneTopic:self.topic];
        }
